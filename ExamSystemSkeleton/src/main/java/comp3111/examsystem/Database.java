...
public class Database<T> {
    ...
    // Query database based on name
    public T queryByName(String name, String field) {
        List<String> slist = FileUtil.readFileByLines(jsonFile);

        T res = null;
        for (int i = 0; i < slist.size(); i++) {
            T t = txtToEntity(slist.get(i));
            Object tvalue = getValue(t, field);
            if (tvalue.toString().equals(name)) {
                res = t;
                break;
            }
        }
    ...
    public T queryByTwoFields(String item1, String field1, String item2, String field2) {
        List<String> slist = FileUtil.readFileByLines(jsonFile);

        T res = null;
        for (int i = 0; i < slist.size(); i++) {
            T t = txtToEntity(slist.get(i));
            Object tvalue1 = getValue(t, field1);
            Object tvalue2 = getValue(t, field2);
            if (tvalue1.toString().equals(item1) && tvalue2.toString().equals(item2)) {
                res = t;
                break;
            }
        }
    ...
    public List<T> queryAllOfKey(String key, String keyname) {
        List<String> slist = FileUtil.readFileByLines(jsonFile);
        List<T> res = new ArrayList<>();
        for (int i = 0; i < slist.size(); i++) {
            T t = txtToEntity(slist.get(i));
            Object tvalue = getValue(t, keyname);
            if (tvalue.toString().equals(key)) {
                res.add(t);
            }
        }
    ...
    // Delete from database by field
    public void delBy2Field(String fieldName1, String fieldValue1, String fieldName2, String fieldValue2) {
        List<T> tlist = getAll();
        for (int i = 0; i < tlist.size(); i++) {
            Object value1 = getValue(tlist.get(i), fieldName1);
            Object value2 = getValue(tlist.get(i), fieldName2);
            if (value1.toString().equals(fieldValue1) && value2.toString().equals(fieldValue2)) {
                tlist.remove(i);
                break;
            }
        }
    ...
    /**
     * This method checks if a given username and password combination exists in the "database"
     *
     * @param username
     * @param password
     * @return
    ...
    public boolean checkUser(String username, String password) {
        List<T> userList = getAll();
        for (T user : userList) {
            String storedUsername = (String) getValue(user, "username");
            String storedPassword = (String) getValue(user, "password");
